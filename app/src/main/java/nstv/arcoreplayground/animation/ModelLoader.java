...
public class ModelLoader {
  ...
  private final WeakReference<AnimationActivity> owner;

  ModelLoader(AnimationActivity owner) {
    this.owner = new WeakReference<>(owner);
  }

  /**
   * Starts loading the model specified. The result of the loading is returned asynchrounously via
   * {@link AnimationActivity#setRenderable(int, ModelRenderable)} or {@link
   * AnimationActivity#onException(int, Throwable)}.
   *
  ...
  boolean loadModel(int id, int resourceId) {
    AnimationActivity activity = owner.get();
    if (activity == null) {
